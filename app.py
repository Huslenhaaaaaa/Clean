...
def main():
    ...
    model_path = "models/xgboost_price_prediction.pkl"
    if os.path.exists(model_path):
        model = load_model(model_path)
        if model:
            st.success("Price prediction model loaded successfully!")
        else:
            st.warning("Failed to load prediction model.")
            model = None
    else:
    ...
    with metrics_col1:
        st.markdown(
            ...
            <div class="card">
                ...
                <div class="metric-value">933,384.36</div>
            </div>
            """, 
            unsafe_allow_html=True
        )
    ...
    with metrics_col2:
        st.markdown(
            ...
            <div class="card">
                ...
                <div class="metric-value">546,523.11</div>
            </div>
            """, 
            unsafe_allow_html=True
        )
    ...
    # Only show prediction visualizations if model is loaded
    if model:
        # Get the property type for predictions
        prediction_type = st.radio(
            "Select property type for prediction:",
            ["Rent", "Sale"],
            horizontal=True
        )
        
        # Filter data based on selected property type
        type_df = df[df['Type'] == prediction_type].copy()
        
        if not type_df.empty and 'Scraped_date' in type_df.columns:
            # Group by date and calculate daily averages
            time_data = type_df.groupby(type_df['Scraped_date'].dt.date).agg({
                'Үнэ': 'mean',
                'ad_id': 'count'
            }).reset_index()
            
            # Check if there's enough data for prediction
            if len(time_data) >= 30:  # Ensure we have at least 30 days of data
                # Sort by date to ensure chronological order
                time_data = time_data.sort_values('Scraped_date')
                ...
                def prepare_data_for_prediction(df):
                    ...
                    df['day'] = df['Scraped_date'].dt.day
                    df['dayofweek'] = df['Scraped_date'].dt.dayofweek
                    
                    # Add lag features (previous days' prices)
                    for i in range(1, 8):  # 1-week lag features
                        df[f'lag_{i}'] = df['Үнэ'].shift(i)
                    
                    # Add rolling averages
                    df['rolling_mean_7'] = df['Үнэ'].rolling(window=7).mean()
                    df['rolling_mean_14'] = df['Үнэ'].rolling(window=14).mean()
                ...
                prepared_data = prepare_data_for_prediction(time_data.copy())
                
                # Generate future dates for prediction (5 months = ~150 days)
                if not prepared_data.empty:
                    last_date = prepared_data['Scraped_date'].max()
                    future_dates = [last_date + timedelta(days=i) for i in range(1, 151)]
                    
                    # Create dataframe for future predictions
                    future_df = pd.DataFrame({'Scraped_date': future_dates})
                    
                    # Extract date features for future dates
                    future_df['year'] = future_df['Scraped_date'].dt.year
                    future_df['month'] = future_df['Scraped_date'].dt.month
                    future_df['day'] = future_df['Scraped_date'].dt.day
                    future_df['dayofweek'] = future_df['Scraped_date'].dt.dayofweek
                    
                    # Initial setup for prediction
                    X_cols = [col for col in prepared_data.columns if col not in ['Scraped_date', 'Үнэ', 'ad_id']]
                    
                    # Make rolling predictions for 5 months
                    # We need to predict one day at a time to generate lag features correctly
                    all_preds = []
                    last_actual_prices = list(prepared_data['Үнэ'].tail(7))  # Get last 7 days of actual prices
                    
                    for i in range(len(future_df)):
                        # Use last known/predicted prices for lag features
                        for j in range(1, 8):
                            future_df.loc[i, f'lag_{j}'] = last_actual_prices[-j]
                        
                        # Calculate rolling means for future date
                        future_df.loc[i, 'rolling_mean_7'] = sum(last_actual_prices[-7:]) / 7
                        future_df.loc[i, 'rolling_mean_14'] = sum(last_actual_prices[-14:]) / 14
                        
                        # Make prediction for this day
                        row_df = future_df.iloc[[i]]
                        prediction = model.predict(row_df[X_cols])
                        all_preds.append(prediction[0])
                        
                        # Update last prices for next iteration
                        last_actual_prices.append(prediction[0])
                    
                    # Add predictions to future dataframe
                    future_df['Үнэ'] = all_preds
                    
                    # Create combined dataframe for visualization
                    historical = prepared_data[['Scraped_date', 'Үнэ']].copy()
                    historical['Type'] = 'Historical'
                    
                    future = future_df[['Scraped_date', 'Үнэ']].copy()
                    future['Type'] = 'Predicted'
                    
                    combined_df = pd.concat([historical, future])
                    
                    # Create prediction plot
                    st.markdown(f"#### {prediction_type} Price Prediction - Next 5 Months")
                    ...
                    fig = px.line(
                        ...
                        y='Үнэ',
                        color='Type',
                        labels={'Scraped_date': 'Date', 'Үнэ': 'Average Price (₮)'},
                        color_discrete_map={'Historical': '#1E40AF', 'Predicted': '#EF4444'}
                    )
                    ...
                    fig.update_layout(
                        title=f"{prediction_type} Price Prediction - Next 5 Months",
                        xaxis_title="Date",
                        yaxis_title="Average Price (₮)",
                    ...
                    # Show projected price changes
                    current_price = prepared_data['Үнэ'].iloc[-1]
                    one_month_price = future_df['Үнэ'].iloc[29]  # ~30 days later
                    three_month_price = future_df['Үнэ'].iloc[89]  # ~90 days later
                    five_month_price = future_df['Үнэ'].iloc[-1]  # ~150 days later
                    
                    one_month_change = (one_month_price - current_price) / current_price * 100
                    three_month_change = (three_month_price - current_price) / current_price * 100
                    five_month_change = (five_month_price - current_price) / current_price * 100
                    
                    # Display projected price changes
                    st.markdown("#### Projected Price Changes")
                    ...
                    with projection_col1:
                        st.markdown(
                            ...
                            <div class="card">
                                ...
                                <div class="metric-value" style="color: {'green' if one_month_change >= 0 else 'red'}">
                                    {one_month_change:.1f}%
                                </div>
                                <div class="metric-label">{one_month_price:,.0f} ₮</div>
                            </div>
                            """, 
                            unsafe_allow_html=True
                        )
                    ...
                    with projection_col2:
                        st.markdown(
                            ...
                            <div class="card">
                                ...
                                <div class="metric-value" style="color: {'green' if three_month_change >= 0 else 'red'}">
                                    {three_month_change:.1f}%
                                </div>
                                <div class="metric-label">{three_month_price:,.0f} ₮</div>
                            </div>
                            """, 
                            unsafe_allow_html=True
                        )
                    ...
                    with projection_col3:
                        st.markdown(
                            f"""
                            <div class="card">
                                <div class="metric-label">5 Month Projection</div>
                                <div class="metric-value" style="color: {'green' if five_month_change >= 0 else 'red'}">
                                    {five_month_change:.1f}%
                                </div>
                                <div class="metric-label">{five_month_price:,.0f} ₮</div>
                            </div>
                            """, 
                            unsafe_allow_html=True
                        )
                    ...
                    with st.expander("About the Price Prediction Model"):
                        st.markdown("""
                        This price prediction model uses XGBoost (eXtreme Gradient Boosting), a powerful machine learning algorithm 
                        for regression tasks. The model was trained on historical price data and makes predictions based on:
                        
                        - **Seasonal patterns**: Day of week, month, and year trends
                        - **Recent price movements**: Using the previous 7 days' prices
                        - **Price momentum**: Using 7-day and 14-day rolling averages
                        
                        The model achieved an R² score of 0.75 on test data, indicating it explains approximately 75% of the 
                        price variation. The Mean Absolute Error (MAE) of ₮546,523 represents the average error in price predictions.
                        
                        *Note: All predictions are estimates and should be used as one of many factors in decision making.*
                        """)
                else:
                    st.warning("Insufficient data for prediction. At least 30 days of historical data is needed.")
            else:
                st.warning(f"Not enough {prediction_type.lower()} property data available for predictions.")
        else:
            st.warning(f"No data available for {prediction_type} properties.")
    else:
        # Add instructions for model training if model isn't available
        with st.expander("How to generate predictions"):
            st.markdown("""
            ### How to Enable Price Predictions
            
            To enable price predictions, you need to:
            
            1. Create a Python script to train the model using the property data
            2. Save the trained model as `models/xgboost_price_prediction.pkl`
            3. The model should be an XGBoost regressor trained to predict price trends
            
            Make sure to create a `models` directory if it doesn't exist already.
            """)
